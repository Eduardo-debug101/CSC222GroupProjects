...
public class MenuInfo {
    ...
    public void loadPresetData(ArrayList<Customer> custList, ArrayList<Plane> planesList) {
        ...
        // Plane 0
        Reservation resv1 = new Reservation(cust1, "C3");
        Reservation resv2 = new Reservation(cust2, "D3");

        // Plane 1
        Reservation resv3 = new Reservation(cust3, "B1");

        // Plane 2
        Reservation resv4 = new Reservation(cust4, "A1");
        Reservation resv5 = new Reservation(cust5, "D4");

        planesList.add(new Plane(LocalDate.of(2022, 12, 1)));
        planesList.add(new Plane(LocalDate.of(2022, 12, 2)));
        planesList.add(new Plane(LocalDate.of(2022, 12, 3)));
    ...
    public void createReservation(ArrayList<Customer> c, ArrayList<Plane> planes) {
        Scanner scan = new Scanner(System.in);
    ...
    public boolean alreadyHasReservation(Customer c, ArrayList<Plane> planes) {
        for(Plane p : planes){
            for (int row = 0; row < p.getSeats().length; row++) {
                for (int col = 0; col < p.getSeats()[row].length; col++) {
                    Reservation current = p.getSeats()[row][col];
                    if(current != null && current.getCust().getId() == c.getId()){
                        System.out.println("You already have a reservation on " + p.getFlightDate() + ". Please cancel this reservation to make another");
                        return true;
                    }
                }
            }
    ...
    public void createSeats(int id, Customer currentCustomer, ArrayList<Plane> planes, ArrayList<Customer> c) {
        Scanner scan = new Scanner(System.in);
        ...
        if (resDate >= 1 && resDate <= 3) { // Date is valid
            ...
            if (!alreadyHasReservation(currentCustomer, planes)) {
    ...
    public void createFirstClassRes(Customer currentCustomer, Plane currentPlane) {
        Scanner scan = new Scanner(System.in);
        ...
        if (num1Row >= 0 && num1Row <= 1 && num1Col >= 0 && num1Col <= 1) { // Is a valid first class seat
            if (currentPlane.getSeats()[num1Row][num1Col] == null) { // Is seat taken
                ...
                Reservation res = new Reservation(currentCustomer, seatChoice);
    ...
    public void createEcoRes(Customer currentCustomer, Plane currentPlane, int ecoSeatType) {
        Scanner scan = new Scanner(System.in);
        ...
        if (ecoSeatType == 1 && (num1Col == 0 || num1Col == 7) && (num1Row == 2 || num1Row == 3)) { // Window seats
            validSeat = true;
        } else if (ecoSeatType == 2 && (num1Col == 3 || num1Col == 4) && (num1Row == 2 || num1Row == 3)) { // Aisle
            // seats
            validSeat = true;
    ...
    public void openSeats(ArrayList<Plane> planes) {
        Scanner scan = new Scanner(System.in);
    ...
    public void cancelReservation(ArrayList<Customer> c, ArrayList<Plane> planes) {
        Scanner scan = new Scanner(System.in);
...
//				}
    ...
    public void cancelSeats(int id, Customer currentCustomer, ArrayList<Plane> planes, ArrayList<Customer> c) {
        Scanner scan = new Scanner(System.in);
        System.out.println("Which date are you inquiring about?  "
                + "Type 1 for 12/1/2022, 2 for 12/2/2022, or 3 for 12/3/2022.");
        int resDate = scan.nextInt();
    ...
    public void saveFile(ArrayList<Plane> planes) {
    ...
    public ArrayList<Customer> readText(ArrayList<Plane> np) {
        ArrayList<Customer> cust = new ArrayList<Customer>();
        // Creates different "Planes" for each date - Eduardo 12/9
        // Plane 0 LocalDate.of(2022, 12, 1)
        np.add(new Plane(LocalDate.of(2022, 12, 1)));
        // Plane 1 LocalDate.of(2022, 12, 2)
        np.add(new Plane(LocalDate.of(2022, 12, 2)));
        // Plane 0 LocalDate.of(2022, 12, 3)
        np.add(new Plane(LocalDate.of(2022, 12, 3)));
